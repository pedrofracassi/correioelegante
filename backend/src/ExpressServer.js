...
module.exports = class ExpressServer {
  static initialize (database) {
    ...
    app.use((req, res, next) => {
      if ((req.method === 'POST' && req.path === '/letters') || (req.method === 'GET' && (req.path.startsWith('/jpeg') || req.path.startsWith('/svg')))) return next()
    ...
    app.get('/svg/:letterId', (req, res) => {
      letterCollection.findOne({ _id: new ObjectId(req.params.letterId) }).then(async doc => {
        if (doc) {
          const jpeg = await LetterRenderer.render(doc, true)
